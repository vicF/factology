// packages/engine/src/utils/networkMonitor.js
//
// Engine module: keep it free of any Vue import. State is exposed as plain
// `{ value }` objects — the same shape Vue refs use, so engine consumers
// (dataLayer, SyncEngine) that read `.value` are unchanged. The app layer
// wraps these in its own reactivity if any UI needs to react to connectivity
// changes.
