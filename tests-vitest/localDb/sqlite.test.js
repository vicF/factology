...
async function createTestAdapter() {
    ...
    const fileIO_ref = fileIO; // keep ref for reset
    await adapter.init('test_db');
    return { adapter, fileIO: fileIO_ref };
}
...
describe('SQLiteAdapter — autoIncrement table (pendingChanges)', () => {
    ...
    it('auto-increments id on put without id', async () => {
        // put without PK for autoIncrement table
        // Use the internal _run to test auto-increment via raw SQL
        adapter._run(
    ...
    it('pendingChanges.put with explicit id works', async () => {
        // bypass autoIncrement by providing an id
        adapter._run(
...
describe('SQLiteAdapter — transaction', () => {
    ...
    it('rolls back on error', async () => {
        ...
        // t2 should have been rolled back
        const t2 = await adapter.objects.get('t2');
...
describe('SQLiteAdapter — save/load persistence', () => {
    it('persists data across close and reload', async () => {
        // Create first adapter, write data, save
        const fileIO = createMemoryFileIO();
        ...
        // Create second adapter reading same buffer
    ...
    it('initializes empty when no file exists', async () => {
        // fileIO that throws on read (no existing file)
        const fileIO = {
...
describe('createBackend — backend selection', () => {
    ...
    it('uses Dexie when no fileIO and no node fs', async () => {
        // We can't easily mock hasNodeFS in this test, but we can test that
        // the default backend (getDb without initDb) gives Dexie.
        // This relies on the existing fallback path.
    });
});
...
describe('initDb/getDb integration', () => {
    it('initDb with fileIO primes the adapter for getDb', async () => {
        ...
        // getDb should return the same adapter
        const db = getDb();
        expect(db).toBe(backend);

        // Should be usable for real operations
